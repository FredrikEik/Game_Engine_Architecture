...
void player::update(float deltaTime)
{
    ...
    if (mMesh)
    {
        ...
        mMesh->getTransformComp()->mTrueScaleMatrix.setPosition(position.x + mDt, mHeight, position.z + (-4 /  deltaTime));
        mMesh->getTransformComp()->mMatrix.setPosition(position.x + mDt, mHeight, position.z + (-4 /  deltaTime));
        mDt = 0.f;
    }
}
