...
void ObjMesh::init()
{
    ...
    glGenBuffers(1, &mMeshComp->mEAB);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mMeshComp->mEAB);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, mMeshComp->mIndices.size() * sizeof(GLuint), mMeshComp->mIndices.data(), GL_STATIC_DRAW);
        ...
        if (oneWord == "f")
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                if (uv > -1)    //uv present!
                {
                    ...
                    mMeshComp->mVertices.push_back(tempVert);
                }
                ...
                mMeshComp->mIndices.push_back(temp_index++);
            }
            continue;
        }
    }
