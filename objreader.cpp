...
void objreader::init(GLint matrixUniform)
{
...
void objreader::draw() //Drawable toggles visibility to objects, and makes pickups invisible.
{
    glBindVertexArray( mVAO );
    ...
    while(std::getline(fileIn, oneLine))
    {
        ...
        if (oneWord == "f")
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                if (uv > -1)    //uv present!
                {
                    Vertex tempVert(tempVertecies[index], tempNormals[normal], tempUVs[uv], tempNormals[normal]);
                    mVertices.push_back(tempVert);
                }
                else            //no uv in mesh data, use 0, 0 as uv
                {
                    Vertex tempVert(tempVertecies[index], tempNormals[normal], QVector2D(0.0f, 0.0f),tempNormals[normal]);
                    mVertices.push_back(tempVert);
                }
